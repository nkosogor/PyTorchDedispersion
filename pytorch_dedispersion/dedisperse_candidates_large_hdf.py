    ...
    if config.get("LOAD_FREQ_BATCH_SIZE", 0) > 0 and file_path.endswith('.hdf5'):
        ...
        for start_idx in tqdm(range(0, total_freq, load_freq_batch_size),
            ...
            if global_summed_data is None:
                global_summed_data = batch_summed_data
            else:
                global_summed_data += batch_summed_data

            

            # Delete GPU tensors and clear cache.
            del processor, data_batch, frequencies, frequencies_tensor, data_tensor, dedisperse_obj, dedispersed_data, batch_summed_data
            if device.type == 'cuda':
